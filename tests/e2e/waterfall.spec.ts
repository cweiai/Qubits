...
test("瀑布流：工具卡片按阶段折叠为块，展开后可见全部真实工具调用", async ({ page }) => {
  ...
  await expect(page.getByTestId("tool-stage-group")).toHaveCount(4, { timeout: 120_000 });

  // After the run: all blocks collapsed by default.
  await expect(page.getByText("应用已生成并通过构建、评审与预览").first()).toBeVisible({ timeout: 120_000 });
  await expect(page.getByTestId("tool-call-card")).toHaveCount(0);

  // Expand each block: all 13 cards visible and successful.
  const groups = page.getByTestId("tool-stage-group");
  for (let i = 0; i < (await groups.count()); i++) {
    await groups.nth(i).getByTestId("stage-toggle").click();
  }
  // Mike: delegate ×4 + render_preview + complete_run；Alex: workspace_init/fs_write×3/run_typecheck/run_tests/run_build。
  await expect(page.getByTestId("tool-call-card")).toHaveCount(13);
  ...
  await expect(page.locator('iframe[data-testid="sandbox-iframe"]')).toBeVisible({ timeout: 120_000 });
});
