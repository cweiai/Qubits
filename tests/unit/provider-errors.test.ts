...
import { openaiProvider, ProviderError, ToolMessageProtocolError } from "@/lib/ai/openai-provider";
 ...
 * retried, user abort never retried, internal timeout → PROVIDER_TIMEOUT, and secret
...
describe("Provider 错误映射（stub fetch，不调用真实模型）", () => {
  ...
  it("内部超时（无 HTTP 响应）→ PROVIDER_TIMEOUT，且不重试", async () => {
    process.env.OPENAI_API_KEY = "sk-test-key-000000000000000000";
