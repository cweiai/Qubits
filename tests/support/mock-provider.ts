import type { AIProvider, AgentTurnResponse, GenerateWithToolsInput } from "@/lib/ai/provider";

/**
 * Test provider: only replaces the external model service when injected by a test;
 * it does not replace tool-call logic — Mike's tool
 * calls, delegation, Alex's REAL fs_write / run_lint / run_typecheck / run_tests / run_build,
 * Alex's quality gates and preview/complete all run the real execution path.
 */
...
const MIKE_SEQUENCE: Array<{ name: string; args: Record<string, unknown> }> = [
  ...
  { name: "delegate_to_agent", args: { targetRole: "engineer", task: "基于产品简报编写并验证真实 React 应用", expectedOutput: "code_workspace", inputArtifactIds: ["$product_brief"] } },
  { name: "render_preview", args: { artifactId: "$preview_bundle", reason: "initial_generation", deviceHint: null } },
  { name: "complete_run", args: { summary: "应用已生成并通过构建、测试、安全扫描与预览", nextSuggestions: ["继续对话修改应用"] } },
];
...
function engineerFinalFromMessages(messages: Array<{ role: string; content: string | null }>): { content: string; toolCalls: []; reasoningContent: null } {
  ...
  let buildArtifactId: string | null = null;
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    try {
      ...
      if (typeof parsed.outputBytes === "number" && (parsed.status === "success" || parsed.status === "failed")) {
        ...
        buildArtifactId = parsed.buildArtifactId ?? null;
